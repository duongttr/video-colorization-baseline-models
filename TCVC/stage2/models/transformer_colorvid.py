...
class Transformer_decoder_parallel(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8,head_warp = 2,
                 ...
                 return_intermediate_dec=False, device=None):                                #True
        super().__init__()
        warp_layer = TransformerMHWarpLayer(512,head=head_warp, device=device)
        decoder_layers = nn.ModuleList()
        # decoder_layers.append(warp_layer)
        decoder_layers.append(TransformerDecoderLayer(384,384, int(nhead/1), 2048,dropout, activation))
        decoder_layers.append(TransformerDecoderLayer(384,384, int(nhead/1), 2048,dropout, activation))
        decoder_layers.append(TransformerDecoderLayer(384,384, int(nhead/1), 2048,dropout, activation))
        # decoder_layers.append(vc_layer(512,384, int(nhead/8), 1024, dropout, activation))
        # decoder_layers.append(vc_layer(512,384, int(nhead/8), 1024, dropout, activation))
        self.decoder = TransformerDecoder_colorvid(decoder_layers)
...
class TransformerMHWarpLayer(nn.Module):
    ...
    def __init__(self, d_model, head=2, device=None):
        super().__init__()
    ...
    def forward(self,
        ...
        #-----------------------------------------------#
        similarity_map_ref = torch.ones([n_k,b,self.head],device=self.device)    # n b 1
        ...
        return src,src_y,src_map#,mask_predict

class TransformerDecoder_colorvid(nn.Module):

    def __init__(self, decoder_layers, norm=None):
        super().__init__()
        self.vc_layer0 = decoder_layers[0]
        self.vc_layer1 = decoder_layers[1]
        self.vc_layer2 = decoder_layers[2]
        ...
        self.fuse_trans2cnn2 = nn.Conv2d(512+384, 512, 1)
        self.fuse_cnn2trans2 = nn.Linear(512+384, 384)
        self.conv6_3norm2 = nn.InstanceNorm2d(512)
        self.fuse_trans2cnn3 = nn.Conv2d(512+384, 512, 1)
        self.fuse_cnn2trans3 = nn.Linear(512+384, 384)
        self.conv7_3norm2 = nn.InstanceNorm2d(512)
  ...
  #----------------------colornet-------------------------#   bn c h w
        ...
        #-----------------------vc layer 1 -----------------------#
        src_cnn2trans = self.fuse_cnn2trans2(torch.cat([src.view(b,n,512,h//8,w//8
            ).permute(1,3,4,0,2).flatten(0,2) ,src_trans],dim=2))
        src_trans = self.vc_layer1(src_cnn2trans,temperature=1)
        src_trans2cnn = src_trans.view(n,h//8,w//8,b,384).permute(3,0,4,1,2).flatten(0,1)
        #---------------------------------------------------------#
        conv6_1 = self.relu6_1(self.conv6_1(src))
        conv6_2 = self.relu6_2(self.conv6_2(conv6_1))
        conv6_3 = self.relu6_3(self.conv6_3(conv6_2))
        conv6_3 = self.conv6_3norm2(conv6_3)
        conv6_3 = self.fuse_trans2cnn2(torch.cat([conv6_3,src_trans2cnn],dim=1))
        conv6_3 += src 
        src = self.conv6_3norm(conv6_3)
        #-----------------------vc layer 2 -----------------------#
        src_cnn2trans = self.fuse_cnn2trans3(torch.cat([src.view(b,n,512,h//8,w//8
            ).permute(1,3,4,0,2).flatten(0,2) ,src_trans],dim=2))
        src_trans = self.vc_layer2(src_cnn2trans,temperature=1)
        src_trans2cnn = src_trans.view(n,h//8,w//8,b,384).permute(3,0,4,1,2).flatten(0,1)
        #---------------------------------------------------------#
        conv7_1 = self.relu7_1(self.conv7_1(src))
        conv7_2 = self.relu7_2(self.conv7_2(conv7_1))
        conv7_3 = self.relu7_3(self.conv7_3(conv7_2))
        conv7_3 = self.conv7_3norm2(conv7_3)
        conv7_3 = self.fuse_trans2cnn3(torch.cat([conv7_3,src_trans2cnn],dim=1))
        conv7_3 += src 
